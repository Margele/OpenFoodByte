/*
 * Decompiled with CFR 0.1.0 (FabricMC a830a72d).
 * 
 * Could not load the following classes:
 *  java.lang.IllegalArgumentException
 *  java.lang.Object
 *  java.lang.String
 */
package awsl;

public abstract class Class63 {
    public static final char Field634 = '+';
    public static final char Field635 = '-';
    public static final char Field636 = '=';
    protected final int Field637;
    private static String Field638;

    public Class63(int a) {
        if (a != 262144 && a != 327680) {
            throw new IllegalArgumentException();
        }
        this.Field637 = a;
    }

    public void Method3610(String a) {
    }

    public Class63 Method3611() {
        return this;
    }

    public Class63 Method3612() {
        return this;
    }

    public Class63 Method3613() {
        return this;
    }

    public Class63 Method3614() {
        return this;
    }

    public Class63 Method3615() {
        return this;
    }

    public Class63 Method3616() {
        return this;
    }

    public Class63 Method3617() {
        return this;
    }

    public void Method3618(char a) {
    }

    public void Method3619(String a) {
    }

    public Class63 Method3620() {
        return this;
    }

    public void Method3621(String a) {
    }

    public void Method3622(String a) {
    }

    public void Method3623() {
    }

    public Class63 Method3624(char a) {
        return this;
    }

    public void Method3625() {
    }

    public static void Method3783(String string) {
        Field638 = string;
    }

    public static String Method3784() {
        return Field638;
    }

    private static IllegalArgumentException Method3785(IllegalArgumentException illegalArgumentException) {
        return illegalArgumentException;
    }

    static {
        if (Class63.Method3784() != null) {
            Class63.Method3783("CKqP0");
        }
    }
}